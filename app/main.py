...
from app.schemas import DirectJourneyResponse, DirectTripOut, IntermediateStopOut, StationOut
...
def _run_direct_query(
    ...
    try:
        origin, destination = validation.validate_query(conn, from_crs, to_crs, date, time)
...
def results(
    ...
    journey = None
    try:
        journey = _run_direct_query(conn, from_, to, date, time, window_minutes)
